...
利用总结在 @tbl-sex-discrimination-obs 中的研究结果，我们希望评估自我认同为女性的个体在晋升决策中是否受到了不公平的歧视。
...
数据在 @fig-sex-rand-obs 中被可视化为一组卡片。
...
@tbl-sex-discrimination-obs 显示，女性人员比男性人员少获得了 7 次晋升，晋升率的差异为 `r perc_diff` $\left( \frac{21}{24} - \frac{14}{24} = 0.292 \right).$ 这个观察到的差异就是我们所说的真实差异的**点估计**\index{point estimate}。
...
@tbl-sex-discrimination-obs 显示，35 位银行主管推荐了晋升，13 位没有。
...
@fig-sex-rand-shuffle-1 突出了洗牌和重新分配到模拟性别组的两个步骤。
...
@tbl-sex-discrimination-rand-1 展示了一次此类模拟的结果。
...
::: {.guidedpractice data-latex=""}
在 @tbl-sex-discrimination-rand-1 中，两个模拟组之间的晋升率差异是多少？
...
@fig-sex-rand-shuffle-1-sort 表明，原始数据中的晋升率差异远大于模拟组中的差异 (0.292 \> 0.042)。
...
@fig-sex-rand-dot-plot 展示了来自 100 次模拟的差异图，其中每个点代表一次模拟中男性和女性文件被推荐晋升的比例差异。
...
::: {.workedexample data-latex=""}
根据 @fig-sex-rand-dot-plot，你观察到至少 `r perc_diff`（`r p_diff`）差异的频率有多高？
经常、有时、很少，还是从不？

------------------------------------------------------------------------

看起来，根据 @fig-sex-rand-dot-plot，在原假设下，出现至少 `r perc_diff` 差异的时间大约只有 2%。
...
@tbl-opportunity-cost-obs 总结了研究结果。
...
@fig-opportunity-cost-obs-bar 显示，与对照组的学生相比，处理组中选择不购买影片的学生比例更高。
...
另一种审视 @tbl-opportunity-cost-obs 中结果的有效方法是使用行比例，具体是考察每组参与者中表示会购买或不会购买影片的比例。
这些汇总数据在 @tbl-opportunity-cost-obs-row-prop 中给出。
...
因为这是一项实验，所以我们将使用与 @sec-caseStudySexDiscrimination 中相同的方法：随机化。
...
单次随机化的结果如 @tbl-opportunity-cost-obs-simulated 所示。
...
结果汇总在 @fig-opportunity-cost-rand-dot-plot 的点图中，每个点代表一次随机化产生的差异。
...
由于点数过多且难以彼此区分，将结果汇总在直方图中更为方便，例如 @fig-opportunity-cost-rand-hist 中的直方图，其中每个柱的高度代表了产生该幅度结果的模拟次数。
...
在 @sec-caseStudySexDiscrimination 中，我们遇到了 20 世纪 70 年代的一项研究，该研究探讨了是否有强有力的证据表明女性候选人获得晋升的可能性低于男性候选人。
...
那么，在 @sec-caseStudyOpportunityCost 中介绍的机会成本研究中，检验统计量是什么？
...
::: {.workedexample data-latex=""}
在 @sec-caseStudyOpportunityCost 的机会成本研究中，我们分析了一项实验，如果提醒研究参与者如果不把钱花在视频上可以用于未来的其他购买，他们继续购买视频的可能性会下降 20%。
...
@tbl-chp11-summary 提供了从另一个角度看的随机化检验总结。
