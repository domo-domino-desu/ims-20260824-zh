...
% 1 Page Parameters
% 1.1
\newcommand{\officialtextheight}{251mm}
\newcommand{\officialtextwidth}{165mm}
% Chinese body text: no extra paragraph gap, two-em first-line indent, and a
% slightly more generous baseline than the Latin-oriented default.
\AtBeginDocument{%
...
% 1.2 Margin Size
% Keep geometry as the sole source of page dimensions.  The old template
% supplied a negative voffset and manual text dimensions, which placed the
% fancyhdr header outside the physical page after switching to A4.
\geometry{
  a4paper,
  inner=25mm,
  outer=20mm,
...
\let\imsoriginaltextbf\textbf
\renewcommand{\textbf}[1]{%
  \ifmmode
    \imsoriginaltextbf{#1}%
  \else
    {\CJKfamily{zhemph}\bfseries #1}%
  \fi}

% Chapter and section titles

% Keep only the appendix letter in the table-of-contents number box.  The
% visible chapter heading is formatted independently by titlesec below.
\ctexset{appendix/name={,}}

% Start every top-level section on a new page except the first section in a
% chapter.  A chapter may contain introductory prose before that first section,
% so track the first section structurally rather than testing page position.
\newif\ifimsfirstsection
\imsfirstsectionfalse
\AddToHook{cmd/chapter/before}{\global\imsfirstsectiontrue}
\AddToHook{cmd/section/before}{%
  \ifimsfirstsection
    \global\imsfirstsectionfalse
  \else
    \clearpage
  \fi
}
...
%\vspace{-2mm}
\setlength{\parskip}{1em}
\noindent\textbf{\color{oiB}\small\CJKfamily{zhemph}{引导练习}} $\:$ \\
}{\end{minipage}
...
%\vspace{-2mm}
\setlength{\parskip}{1em}
\noindent\textbf{\color{oiB}\small\CJKfamily{zhemph}{范例}} $\:$ \\
\small}{\end{minipage}
\end{mdframedwithfootGPWE}
\vspace{4mm}
}
...
\vspace{-2mm}
\setlength{\parskip}{1em}
\noindent\textbf{\color{oiGray}\small\CJKfamily{zhemph}{待办}} $\:$ \\ \\
}{\end{minipage}
\end{mdframed}
\vspace{4mm}
}
...
\vspace{-2mm}
\setlength{\parskip}{1em}
\noindent\textbf{\color{oiR}\small\CJKfamily{zhemph}{尚在编写}} $\:$ \\ \\
}{\end{minipage}
\end{mdframed}
\vspace{4mm}
}
...
\usepackage{tocbasic}
% Chapter headings already spell out “第…章” in the body.  In the TOC,
% use the compact Arabic number used by the original book.
