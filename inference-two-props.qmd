...
让我们再看一看在 @sec-two-sided-hypotheses 中介绍过的心肺复苏（CPR）研究。
...
结果汇总在 @tbl-cpr-summary-again 中（它是 @tbl-cpr-summary 的副本）。
...
我们将观测随机分配到两组的过程在 @fig-fullrand 中进行了总结和可视化。

### 统计量的变异性

@fig-cpr-rand-dot-plot 显示了从 100 次随机化模拟（即 @fig-fullrand 所描述的重复迭代）中得到的差值的堆叠点图，其中每个点代表一次模拟中感染率的差值（对照组比例减去处理组比例）。
...
根据 @fig-cpr-rand-dot-plot，你会多频繁地观察到至少 13%（0.13）的差异？
这是一个罕见事件吗？

------------------------------------------------------------------------

根据 @fig-cpr-rand-dot-plot，如果原假设为真，仅由随机性造成至少 13% 的差异约有 12% 的概率。这不是一个非常罕见的事件。
:::
...
在 @sec-two-prop-errors 中，我们利用随机化分布来理解当原假设 $H_0: p_1 - p_2 = 0$ 成立时 $\hat{p}_1 - \hat{p}_2$ 的分布。现在，通过自助法，我们研究 $\hat{p}_1 - \hat{p}_2$ 的变异性，而无需假设原假设成立。

### 观测数据

重新考虑来自 @sec-two-prop-errors 的 CPR 数据，该数据由 @tbl-cpr-summary 提供。同样，我们使用样本比例之差作为感兴趣的观测统计量。此处，统计量的值为：$\hat{p}_T - \hat{p}_C = 0.35 - 0.22 = 0.13.$

### 样本比例差异的变异性

应用于两个样本的自助法是 [第 -@sec-foundations-bootstrapping 章] 中所述方法的扩展。现在，我们有两个样本，因此每个样本都估计它们所来源的总体。在 CPR 研究中，`treatment` 样本估计所有已经（或将要）接受处理的个体总体；`control` 样本估计所有不接受处理作为对照的个体总体。@fig-boot2proppops 扩展了 @fig-boot1，以展示同时从两个样本进行自助法的过程。
...
与之前一样，一旦估计了总体，我们就可以随机对观测进行重抽样来创建自助样本，如 @fig-boot2propresamps 所示。
...
与以往一样，比例之差的变异性只能通过重复模拟来估计，在本例中，也就是重复的自助重抽样。@fig-boot2samp2 展示了针对每次重复的自助抽样计算出的多个自助差值。
...
@fig-boot2samp1 可视化了这一过程，而 @fig-bootCPR1000 展示了 CPR 数据的 1000 个自助比例之差。
请注意，CPR 数据中两个组别的人数分别为 40 和 50，而图示例子中两组的人数分别为 7 和 9。
因此，在图示的例子中，样本比例分布的变异性更高。
正如您将在 @sec-math-2prop 中讨论的数学模型中所见，较大的样本量会导致比例之差的标准误更小。
...
@fig-bootCPR1000 提供了对样本间存活比例之差的变异性的估计。
...
正如在 [第 -@sec-foundations-bootstrapping 章] 中所述，自助置信区间可以直接根据 @fig-bootCPR1000 中的自助差值计算得出。
...
@sec-math-2prop 详细介绍了样本比例之差标准误的数学模型，但自助分布通常能极好地估计样本统计量抽样分布的变异性。
...
对于其他百分比，例如 90% 自助标准误置信区间，我们将使用标准正态分布给出的分位数，如 @sec-normalDist 和 @fig-er6895997 所示。
...
@fig-ci25ints 展示了一个假设场景：在完全相同的总体（目标都是估计真实参数值 $p_1 - p_2 = 0.47$）上进行了 25 项不同的研究。手头的研究对应一个点估计（一个点）及其相应的区间。
我们不可能知道手头的这个区间是位于未知的真实参数值（黑色虚线）的右侧还是左侧。
也无从知晓该区间是否捕获了真实参数（蓝色）或者没有（红色）。
如果我们构建的是 95% 置信区间，那么在我们一生中创建的区间中，大约有 5% *不会*捕获感兴趣的参数（例如，在 @fig-ci25ints 中会是红色）。
...
我们将考虑 @tbl-fish-oil-data 所列患者中的心脏病发作结果。
...
在接下来的 25 年研究期间没有进行干预，我们将考虑在整个 30 年期间因乳腺癌导致的死亡。研究结果总结在 @tbl-mammogramStudySummaryTable 中。
...
描述乳腺 X 光检查的研究问题是为了检验特定的假设（这与参数的置信区间形成对比）。为了充分利用假设检验的结构，我们在原假设为真的条件下评估随机性（正如我们在假设检验中一贯的做法）。使用 @tbl-mammogramStudySummaryTable 中的数据，我们将检查使用正态分布通过假设检验来分析研究结果的条件。
...
[^17-inference-two-props-5]: 有关不要求满足成功—失败条件的两比例假设检验的示例，请参见 @sec-two-prop-errors。
