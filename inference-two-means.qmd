...
学生在这两场考试中表现的汇总统计量如 @tbl-summaryStatsForTwoVersionsOfExams 所示，并在 @fig-boxplotTwoVersionsOfExams 中以图形展示。
...
基于 @fig-rand2means 的思路，@fig-randexams 展示了在 1,000 次随机模拟中模拟统计量 $\bar{x}_{1, sim} - \bar{x}_{2, sim}$ 的值。
...
通过在 @fig-randexamspval 图中标出数值3.1，我们可以衡量3.1与原假设下生成的随机化差值有多大的差异或相似程度。
...
估算 @fig-randexamspval 中描绘的 p 值，并在案例研究的背景下给出结论。
...
@fig-bootmeans2means 直观地描述了当关注的统计量基于两个独立样本计算时的自助过程。
...
@tbl-statsSheepEscStudy 包含了一项在心脏病发作的绵羊身上测试 ESCs 的实验的汇总统计量。
这些绵羊中的每一只都被随机分配到 ESC 组或对照组，并在研究中测量了它们心脏泵血能力的变化。
[@Menard:2005] @fig-stem-cell-histograms 提供了两个数据集的直方图。
...
正如我们在 @sec-two-prop-boot-ci 中所见，我们将使用自助法来估计在重复抽样时与样本均值差相关的变异性。
这种方法类似于两个比例的情形：分别从每组（此处是 ESCs 组和对照组）*分别*有放回抽取一个样本，计算样本均值，然后取它们的差值。
重复整个过程多次，即可产生样本均值差的自助分布（*无需*原假设的假定）。

@fig-bootexamsci 显示了均值差的变异性，并叠加绘制了 90% 百分位置信区间和标准误置信区间。
...
该数据集中的四个观测单位如 @tbl-babySmokeDF 所示。
...
@tbl-births14-summary-stats 展示了数据中的样本统计量。
...
-   由于两组都超过 30 个观测，我们在 @fig-babySmokePlotOfTwoGroupsToExamineSkew 中检查数据，看是否有特别极端的离群值，结果没有发现。
...
::: {.guidedpractice data-latex=""}
@tbl-births14-summary-stats 中的汇总统计量可能对本次引导练习有帮助。
...
> *这是真的。吸烟女性生下的婴儿更小，但他们和从不吸烟女性生下的婴儿一样健康。而且有些女性更愿意生小一点的婴儿。* —— 菲利普莫里斯公司董事会主席约瑟夫·卡尔曼，于 CBS《面对全国》节目，1971年1月3日

此外，母亲吸烟与不吸烟所生婴儿之间的健康差异并不仅限于体重差异。[^20-inference-two-means-6]

[^20-inference-two-means-6]: 你可以观看约翰·奥利弗在《上周今夜秀》上的一期节目 [*Last Week Tonight*](youtu.be/6UsHHOCH4q8)，了解烟草行业现今的劣迹。
    请注意，其中含有一些成人语言。

关于独立样本 t 检验的检验功效的一个小注记（回顾 @sec-pow 节中关于检验功效的讨论）。事实证明，这里给出的独立样本 t 检验的检验功效通常低于 @sec-mathpaired 节中讨论的配对 t 检验。不过，取决于数据收集的方式，我们并不总是有对数据进行配对的机制，从而降低观测间固有的变异性。
...
@fig-stem-cell-histograms 并未显示任何一组中存在明显的离群值。
